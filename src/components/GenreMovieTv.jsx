...
function GenreMovieTv({ movieId }) {
  ...
  const filteredGenre_1 = genre?.find(
    (item) => genresToFind?.[0] !== undefined && item.id === genresToFind[0]
  );
  //console.log(filteredData_1);

  const filteredGenre_2 = genre?.find(
    (item) => genresToFind?.[1] !== undefined && item.id === genresToFind[1]
  );
